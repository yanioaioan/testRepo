...
class ModelFitter:
  ...
  def __init__(self, asm, image, originalShapes, t=Point(5,25)):#x,y
    ...
    self.image = image
    #the array of grey_scaled_images
    self.g_image = []
    ...
    for s in (originalShapes):
        ...
        for i,pt in enumerate(s.pts):
            ...
            cv.Circle(oneTrainingImageVectorimage, ( int(pt.x), int(pt.y) ), 4, (100,100,100))
            cv.NamedWindow("TEST", cv.CV_WINDOW_AUTOSIZE)
            cv.ShowImage("oneTrainingImageVectorimage",oneTrainingImageVectorimage)
    ...
    for shape in self.asm.shapes:
        ...
        #now transform each asm read in shape (manually) to where the target vertebra in the image target is (just to visualize it there ..should be placed based on tracking at a later stage)
...
####cv.WaitKey()
  ...
  def calculateIthImageLandmarkDerivIntensityVec(self, p, norm, greyImage, gaussianMatrix, _searchProfiles, IthImage_NormalizedtmpLandmarkDerivativeIntensityVector):
      ...
      for side in range(-3,4):#along normal profile
          ...
          for t in drange(-6, 7, 1):#tangent
              ...
              gradientIntensity=greyImage[scale][y,x] * gaussianMatrix[t+6][side+3]
      ...
      yij_normalizedDerivProfile=np.array(dg_landmarkDerivativeProfiles) / np.array(sum_dg[0])
  ...
  def getCorrectedLandmarkPoint(self):
      ...
      for s in self.asm.shapes:#self.originalShapes
          ...
          #s=[]
          #s.add_point(Point(100,100))
          #s.add_point(Point(200,200))
          #s.add_point(Point(300,300))

          #print "asm shape x&y length flattend (..so divide by 2 to get the total coordinate points) : %s"%( len(s.get_vector()) )
          #print "asm shape : %s"%( s.get_vector() )
          #cv.WaitKey()

          '''get a vector of all landmark points of this shape'''
          oneTrainingImageVector=Shape.from_vector(s.get_vector())#[320,185,309,196,292,203,276,209,264,217,256,237,261,252,275,255,294,243,308,238,327,240,345,241,342,224,336,217,335,209,348,203,359,210,369,214,376,206,392,211,400,215,404,214,402,198,399,189,390,181,389,172,383,160,373,147,354,145,338,161]
          totalLandmarksLeftToBeTested=len(oneTrainingImageVector.pts)



          '''for each of the shape's landmarks'''
          for i,p in enumerate(oneTrainingImageVector.pts):

              #print "p.x=%d ,p.y=%d"%(p.x,p.y)
              ##cv.WaitKey()

              '''draw them in a windows named oneTrainingImageVectorimage'''
              tmpP = Point(p.x, p.y)
              cv.Circle(oneTrainingImageVectorimage, ( int(tmpP.x), int(tmpP.y) ), 4, (100,100,100))
              cv.NamedWindow("oneTrainingImageVectorimage", cv.CV_WINDOW_AUTOSIZE)
              cv.ShowImage("oneTrainingImageVectorimage",oneTrainingImageVectorimage)
              #cv.WaitKey()
              ...
              norm = s.get_normal_to_point(i)#??? self.shape
              ...
              self.calculateIthImageLandmarkDerivIntensityVec(p, norm, greyImage, gaussianMatrix, _searchProfiles, IthImage_NormalizedtmpLandmarkDerivativeIntensityVector)
      ...
      for index in range(len(AllImages_NormalizedtmpLandmarkDerivativeIntensityVector)):
          for i in range(len(AllImages_NormalizedtmpLandmarkDerivativeIntensityVector[index])):
              meancounter+=1
              if not y_j_mean:
                  y_j_mean.append( np.array(AllImages_NormalizedtmpLandmarkDerivativeIntensityVector[index][i]))
              else:
                  y_j_mean[0]=y_j_mean[0]+ np.array(AllImages_NormalizedtmpLandmarkDerivativeIntensityVector[index][i])
              ###print"np.array(profile)=%s"%(np.array(profile))
              ###print"\sum_dg=%s\n"%(sum_dg)
              ####cv.WaitKey()

      #print "meancounter=%d"%(meancounter)
      y_j_mean = np.array(y_j_mean) / meancounter#len(self.asm.shapes)
      ...
      if totalLandmarkNumber==-1:
          totalLandmarkNumber=len(AllImages_NormalizedtmpLandmarkDerivativeIntensityVector[0])#for 30 totalLandmarkNumber

      #for s in self.asm.shapes:

      correctionVectorForAllShapes=[]
      #for p_num in range(totalLandmarkNumber):#0..29 iterate only for 30 elements, but for a different oneTrainingImageVector
      for index in range(len(AllImages_NormalizedtmpLandmarkDerivativeIntensityVector)):

          #get a vector of all landmark points of each image
          oneTrainingImageVector=self.shape#Shape.from_vector(self.asm.shapes[index].get_vector())#self.originalShapes

          #print 'oneTrainingImageVector=%s'%(oneTrainingImageVector.pts[0])

          for p_num in range(len(AllImages_NormalizedtmpLandmarkDerivativeIntensityVector[index])):
              ...
              #x is the intensity vector for this profile
              x=np.array(AllImages_NormalizedtmpLandmarkDerivativeIntensityVector[index][p_num])
              #y is the mean intensity vector for the 6 profiles (shouldn't this be 7 instead of 6 profiles ??? )
              y=np.array(y_j_mean[0])
              ...
              print "covar=%s"%(covar[0])
              #cv.WaitKey()

              #invcovar = np.linalg.inv(covar.reshape((13,13)))

              invcovar=0
              invcovar=cv2.invert(covar, invcovar, cv2.DECOMP_SVD)#'''covar'''
              invcovar=invcovar[1]
              ...
              ##print"oneTrainingImageVector=%d"%(len(oneTrainingImageVector.pts))
              ##print"\n\npoint number=%d\n\n"%(p_num)
              norm = oneTrainingImageVector.get_normal_to_point(p_num)
              ##print"norm\n",norm
              ...
              p = oneTrainingImageVector.pts[p_num]#1st point ..2nd point.. and so on
              ##print"p_num%d"%(p_num)
              ###cv.WaitKey()
              ##print"p=%s"%(p)
              ###cv.WaitKey()

              ##print"Points of the mean shape of the Trained Set",oneTrainingImageVector.pts
              ...
              if len(correctionVectorForAllShapes)<len(oneTrainingImageVector.pts):#write the fist 30 landmarks
  ...
  def do_iteration(self, scale, i):
    ...
    self.calcMaxPtFromMahalanobisModelProfile(self.shape,scale,i)



  def calcMaxPtFromMahalanobisModelProfile(self, initmeanshape, scale,i):
    ...
    max_ptlist=self.getCorrectedLandmarkPoint()
