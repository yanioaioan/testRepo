...
class FitModel:
  ...
  """
  def __init__(self, asm, image, targetImageName, originalShapes, t=Point(10,-5)):#x,y #5,85 for c3
    ...
    self.currentSearchPyramidLevel=2
    ...
    pyramidLevels=3
    for imagelevel in range(pyramidLevels):
        ...
        if imagelevel == 0:
            self.greyTargetImage.append(self.__produce_gradient_image(image, 2**imagelevel))
            ...
            if imageNameDownSampled==0:
                img = cv2.imread(targetImageName)
            ...
            for iterativeSamplingIndex in range(imagelevel):
                ...
                cv2.imshow("target_grey_imageDownSampled_%d.jpg"%(imagelevel),img)
                cv2.imwrite("target_grey_imageDownSampled.jpg",img)
                test_grey_image = cv.LoadImage("target_grey_imageDownSampled.jpg")
                cv.WaitKey()
...
####cv.WaitKey()
  ...
  def findLandmarkNormIIntesityVec(self, gMeanFlag, oneTrainingImageVector, shape, trainingSetImage, IthImage_NormalizedtmpLandmarkDerivativeIntensityVector,gaussianMatrix, greyImage):
      ...
      #***this vector contains ALL the normalized derivative profiles for all landmarks of this shape***
  ...
  def findtrainingGmean(self, testImageNameCounter, AllImages_NormalizedtmpLandmarkDerivativeIntensityVector, gaussianMatrix):
      ...
      for imagelevel in range(pyramidLevels):
          ...
          #grab a vector of points from original shapes landmarked
          for asmshape in self.originalShapes:#self.asm.shapes !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 3 for now
              ...
              if imagelevel == 0:
                  ...
                  #if we are creating the 1 pyramid level sampling
                  if imageNameDownSampled==0:
                      img = cv2.imread(currentImageName[0])
                  #else:
                  ...
                  for iterativeSamplingIndex in range(imagelevel):
                      ...
                      cv2.imwrite("grey_imageDownSampled_level_%d.jpg"%(testImageNameCounter),img)
                      test_grey_image = cv.LoadImage("grey_imageDownSampled_level_%d.jpg"%(testImageNameCounter))
              ...
              cv.Copy(self.g_image[ (testImageNameCounter-1) +(imagelevel*len(self.originalShapes))], trainingSetImage)
          ...
          for i in range(len(AllImages_NormalizedtmpLandmarkDerivativeIntensityVector[0])):#for each landmark in the set (30 for now)
            ...
            for index in range(len(AllImages_NormalizedtmpLandmarkDerivativeIntensityVector)/pyramidLevels):#for each image in the set (3 for now)
                ...
                meancounter+=1
                if len(y_j_mean)==0:
                    y_j_mean.append( np.array(AllImages_NormalizedtmpLandmarkDerivativeIntensityVector[index+(pyramidLevels*imagelevel)][i]))#append 6 profiles with 13 elements each one
                else:
                    y_j_mean[0]=y_j_mean[0]+ np.array(AllImages_NormalizedtmpLandmarkDerivativeIntensityVector[index+(pyramidLevels*imagelevel)][i])#add the next 6 profiles of the of the same landmarks in training set of images
                ####print"np.array(profile)=%s"%(np.array(profile))
                ###print"\y_j_mean=%s\n"%(y_j_mean[0])
                ##cv.WaitKey()
  ...
  def findcurrentShapeGmean(self, currentshape, One_NormalizedtmpLandmarkDerivativeIntensityVector, gaussianMatrix, currentSearchPyramidLevel):
      ...
      '''get a vector of all landmark points of this CURRENT shape'''
      ...
      self.findLandmarkNormIIntesityVec (gMeanFlag, currentshapePointVector, currentshape, self.image, curShape_NormalizedtmpLandmarkDerivativeIntensityVector,gaussianMatrix, self.greyTargetImage[currentSearchPyramidLevel])
...
#                  ####cv.WaitKey()
      ...
      if not self.CovarianceMatricesVec[currentSearchPyramidLevel]:
          ...
          for p_num in range(len(One_NormalizedtmpLandmarkDerivativeIntensityVector[0])):
              ...
              print "tmpCovar=%s"%(tmpCovar)
              cv.WaitKey()
              print "x=%s"%(x)
              cv.WaitKey()

              self.CovarianceMatricesVec[currentSearchPyramidLevel].append(tmpCovar)
              #use the already calculated corresponding covariance matrix

              #print "\nCOVARIANCE_CALCULATION_ONCE"

      print "len of cov %d is %d \n = %s"%(currentSearchPyramidLevel, len(self.CovarianceMatricesVec[currentSearchPyramidLevel]), self.CovarianceMatricesVec[currentSearchPyramidLevel])
      ...
      ####print"\n np.array(y_j_mean)=%s"%(np.array(y_j_mean[0]).flatten())
          ...
          #for j in range(len(x)):
              ...
              #check to see: which of the Xj profiles of the current shape analysed will match best the Yj (training G mean profiles)
              #The tmpMahalDist will be updated every time
              ...
              if bestProfileMatchWithSmallerMahalanobisDistanceFrom_gProf_to_meanProf==-1:
                  ...
                  if tmpMahalDist < bestProfileMatchWithSmallerMahalanobisDistanceFrom_gProf_to_meanProf:
                      bestProfileMatchWithSmallerMahalanobisDistanceFrom_gProf_to_meanProf= tmpMahalDist
                      #SAVE THE j WITH THE SMALLEST tmpMahalDist AS THE bestIndexProfile
                      bestIndexProfile=j
          ...
          side=  2*(bestIndexProfile-3)+1#bestIndexProfile-3
          ...
          ninentyPerCent=9.0/10.0
          halfShapeProfile=(x.shape[0]/2)

          if (bestIndexProfile < (halfShapeProfile )) and self.currentSearchPyramidLevel >=1:#as it can't go further down than 0 level in res pyramid
              ...
              if bestIndexWithinHalfProfileLengthCounter == float(len(One_NormalizedtmpLandmarkDerivativeIntensityVector[0])*ninentyPerCent):
                  ...
                  print "bestIndexWithinHalfProfileLengthCounter == (len(One_NormalizedtmpLandmarkDerivativeIntensityVector[0])*ninentyPerCent): %d=%d"%(bestIndexWithinHalfProfileLengthCounter , (len(One_NormalizedtmpLandmarkDerivativeIntensityVector[0])*ninentyPerCent))
                  cv.WaitKey()

                  self.currentSearchPyramidLevel-=1
                  breaktonextpyramidlevel=True

                  print "MOVE TO NEXT PYRAMID LEVEL AS a sufficient number of landmarks are reached (in this case we chose half of the total number of landmarks)[pyramidlevel:%d]"%(self.currentSearchPyramidLevel)
                  cv.WaitKey()
          ...
          # Look 6 pixels to each side too
          for t in drange(-6, 7, 1):#tangent not really (as at 1st currently, it chooses the directions side along normal and then from there onwards it searches around for nearby strong intensity edges to fit to)
              ####print"t...........",t
              ...
              if targetImageTocheckAgainst[y-1, x-1] > max_edge:
                ...
                cv.Circle(imgtmp, ( int(max_pt.x), int(max_pt.y) ), 10, (100,100,100))
...
return correctionVectorForAllShapes
  ...
  def calcMaxPtFromMahalanobisModelProfile(self, initmeanshape, scale, iterationIndex ,GTrainingMeansCalculated):
    ...
    #the b parameters to ensure we fit the current model instance to the image points correctly
    ...
    for i in range(len(self.asm.evecs.T)):
      #b = Ptransposed (Y-Mean) - b is a k dimensional vector (k being the number of modes) which expresses the small set of parameters to be adjusted instead of the 2n vector of points so as to fit the model to the target image
