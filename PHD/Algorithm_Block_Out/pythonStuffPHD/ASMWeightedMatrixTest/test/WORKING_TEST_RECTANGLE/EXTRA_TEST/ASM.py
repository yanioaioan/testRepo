...
######cv.WaitKey()
  ...
  def findtrainingGmean(self, testImageNameCounter, AllImages_NormalizedtmpLandmarkDerivativeIntensityVector, gaussianMatrix):
      ...
      for imagelevel in range(self.pyramidLevels):
          ...
          ##print"\n\length of AllImages_NormalizedtmpLandmarkDerivativeIntensityVector vector=%d\n"%(len(AllImages_NormalizedtmpLandmarkDerivativeIntensityVector)/self.pyramidLevels)
          ##cv.WaitKey()
          #3 shapes
          ##print"\n\ AllImages_NormalizedtmpLandmarkDerivativeIntensityVector[0] vector=%s\n"%((AllImages_NormalizedtmpLandmarkDerivativeIntensityVector[0]))
          #cv.WaitKey()
...
#                  ######cv.WaitKey()
      ...
      #####print"\n np.array(y_j_mean)=%s"%(np.array(y_j_mean[0]).flatten())
          ...
          side= 2*(bestIndexProfile-eithersideOfNormalPixels)+1 # (-5..-3..-1...1...3..5..)  , #bestIndexProfile-3
          ...
          for j in range(x.shape[0]):
              mypoint = Point(p.x + j*norm[0], p.y + j*norm[1])
          ...
          if targetImageTocheckAgainst[new_p.y-1, new_p.x-1] > max_edge:
            max_edge = targetImageTocheckAgainst[new_p.y-1, new_p.x-1] #greyImage[scale][y, x]
            max_pt = Point(new_p.x , new_p.y) #x,y   #new_p.x + t*norm[0], new_p.y + t*norm[1]
          '''
