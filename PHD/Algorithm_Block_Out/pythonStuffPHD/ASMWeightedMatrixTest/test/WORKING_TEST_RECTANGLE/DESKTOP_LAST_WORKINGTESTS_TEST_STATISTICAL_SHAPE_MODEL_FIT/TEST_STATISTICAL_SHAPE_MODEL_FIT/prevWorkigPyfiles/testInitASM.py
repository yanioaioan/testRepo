...
BRIGHTNESS_DECREASE_COEF = 100#50
CONTRAST_DECREASE_COEF = 2.5#1.5
...
class FitModel:
  ...
  #def __init__(self, asm, image, t=Point(140,45.0)):
              ...
              if not USE_PYRAMID_SMOOTHING:
                ...
                if i==0:
                  ...
                  currentSmoothedLevelTarget=self.pyramid(imageNames[imagenumber],i,  imagenumber, i)
                    ...
                    currentSmoothedLevelTarget=self.pyramid("testPyr_%s.jpg"%(i-1),i-1)
    ...
    answer = "initShapes"#raw_input("where to read from under initShape ??")
  ...
  def __produce_gradient_image(self, i, scale):
    ...
    if ENHANCE_IMAGE:
        ...
        img = cv2.imread('BeforeDerivative-grey_image.jpg',0)  # mandrill reference image from USC SIPI
        print img
        #img =self.apply_brightness_contrast(img, 0, 0)
        equ = cv2.equalizeHist(img)
        res = np.hstack((img,equ)) #stacking images side-by-side

        cv2.imwrite('BeforeDerivative-grey_image.jpg',img)
        grey_image = cv.LoadImage('BeforeDerivative-grey_image.jpg', cv.CV_LOAD_IMAGE_GRAYSCALE)
        #cv.NamedWindow('BeforeDerivative-grey_image.jpg', cv.CV_WINDOW_NORMAL)
        #cv.ShowImage('BeforeDerivative-grey_image.jpg',grey_image)
        #cv.WaitKey()
