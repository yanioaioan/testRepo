...
#was 3 6
#was 3 7 worked for frame_16 but not for frame_46
#was 5 , 6
...
#5,8 fast , seemed to have worked
#5,12 checking

#How many extra pixels in each direction
#STEP=7# NEEDS TO BE 9 when working with 10 frames sequence (10only_v1_half_res).   ----   sample every "STEP" pixels along the whisker # was 10 WORKS FOR FRANK-->6 works for fluoroTest
STEP=5# 5 was sort of working
#Normal Profile range on each side
#PIXELS_SEARCH=16# NEEDS TO BE 5 when working with 10 frames sequence (10only_v1_half_res).   ----    #COULD BE 2 # was 4    #6 work with   __createStatisticalProfileModel() & and self.__get_MAHALANOBIS()
#NOTE: Stasm used 4 for PIXELS_SEARCH on each side & 2 for STEP
PIXELS_SEARCH=12 #10 was sort of working
...
#DEBUG_LINES=1
#STOP_AT_THE_END_OF_EVERY_LANDMARK=1
#SHOW_EVERY_POSSIBLE_POINT=1
#SHOW_EVERY_POSSIBLE_STATISTICAL_SAMPLE=1
...
class ShapeViewer ( object ):
  ...
  def draw_model_fitter(f,scale):
    ...
    for pt_num, pt in enumerate(f.shape.pts):
      ...
      cv.Circle(i, (int(pt.x), int(pt.y)), 1, levelColour, -1)#Un Hash to have points
      nextPoint = (int(pt.x), int(pt.y))

      if prevPoint != -1:#draw line connecting every other point with the next one , except for the last to the first segment
          cv.Line(i, prevPoint , nextPoint ,levelColour,1)
...
class FitModel:
  ...
  #def __init__(self, asm, image, t=Point(140,45.0)):
        ...
        for i in range(0,TOTALLEVELS):#for 4 different levels of sobel derivative smoothing
              ...
              if not USE_PYRAMID_SMOOTHING:
                ...
                if i==0:
                  ...
                  currentSmoothedLevelImage=self.pyramid(imageNames[imagenumber],i,  imagenumber, i)

                else:
                    #currentSmoothedLevelTarget=self.pyramid("testPyr_%s.jpg"%(1),i+1)
                    currentSmoothedLevelImage=self.pyramid("testPyr_%s.jpg"%(i-1),i)


                result_sobelTestPyr=self.__produce_gradient_image(currentSmoothedLevelImage,2**i)
                #cv.NamedWindow("result_sobelTestPyr_level%s"%(i+1), cv.CV_WINDOW_NORMAL)
                #cv.ShowImage("result_sobelTestPyr_level%s"%(i+1),result_sobelTestPyr)
    ...
    #answer =   'initShapes/ANDERST-ADAM/C5/96' #'initShapes/ANDERST-ADAM/C5/96/512x512' #'initShapes' #'initShapes/ANDERST-ADAM/C5/51-75'#raw_input("where to read from under initShape ??")
    #answer =   'initShapes/lastworking'

    answer =   'initShapes/lastworking'
    #answer =   'initShapes/lastworking/abitoff/offseted/alloffseted'
    ...
    print 'checking dir -> %r'%( str(cwd) +str('/') + str(answer) +str('/') +  "frame" + str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]))

    print 'cwd',cwd
    print 'answer',answer
    print 'str(sys.argv[2].split("/")[-1:][0].split(".")[0].split("_")[-1])',(str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]))

    #if there's nothing under initshapes corresponding folder,  manually transform and init
    if len(os.listdir(str(cwd) +str('/') + str(answer) +str('/') +  "frame" + str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]) ) )==0:
      ...
      if len(os.listdir( str(cwd) +str('/') + str(answer) +str('/') +  "frame" + str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]) ))==0:#if initShapes dir empty        
        if not os.path.exists( str(cwd) +str('/') + str(answer) +str('/') +  str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]) +str('/') +   "frame" + str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]) + ".pts" ):


          print str(cwd) +str('/') + str(answer) +str('/') +  "frame" + str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]) +str('/') +   "frame" + str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]) + ".pts"
          cv.WaitKey()


          for initShape in finalPlacedShapeList:
            with open( str(cwd) +str('/') + str(answer) +str('/') +  "frame" + str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]) +str('/') +   "frame" + str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]) + ".pts"   , 'w') as initShapesFile:

              print "writing to.."
              print str(cwd) +str('/') + str(answer) +str('/')  + str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1])
      ...
      #Now read initShapesFile that was just exported back..
      shapes = PointsReader.read_directory( str(cwd) +str('/') + str(answer) +str('/') +  "frame" + str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]) +str('/') +   "frame" + str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]) + ".pts" )      
      for shape in shapes:
          ...
          self.shape =tmpShape#.transform(t, degreesOfRotation, CenterOfMass))
          print self.shape
          ...
          targetCopy = cv.CreateImage(cv.GetSize(self.g_images[frame*TOTALLEVELS+scale]), self.target.depth, 1)
          cv.Copy(self.g_images[frame*TOTALLEVELS+scale], targetCopy)
      ...
      print 'read from --> %s'%( str(cwd) +str('/') + str(answer) +str('/') +  "frame" + str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]) )

      shapes = PointsReader.read_directory( str(cwd) +str('/') + str(answer) +str('/') +  "frame" + str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]) )
  ...
  def do_iteration(self, scale, reset_giAllPointsShapeVecSet):
        ...
        if reset_giAllPointsShapeVecSet==1:
            ...
            if not os.path.exists( "trainingSetProfiles/"+stringfileName+"_"+str(STEP)+"_"+str(PIXELS_SEARCH)+"_trainingSetLevel_%d.txt"%(scale) ):
                ...
                for line in lines:
                    ...
                    #print 'len(self.originalShapes[0].pts)',len(self.originalShapes[0].pts)
    ...
    for i in range(len(self.asm.evecs.T)):
      ...
      #print "new_shape.get_vector() - self.asm.mean=",new_shape.get_vector() - self.asm.mean
      #cv.WaitKey()
    ...
    '''
    for i in range(len(self.asm.evecs.T)):
      for j in range(len(self.asm.evecs)):
          #update parameters to fit the current shape
          #deviation of current shape from mean shape: --> new_shape.get_vector() - self.asm.mean
          #The vector b defines a set of parameters of a deformable model
          b = np.dot(self.asm.evecs[:,i], new_shape.get_vector() - self.asm.mean)
          #max_b = 2*math.sqrt(self.asm.evals[i])
          #constrain shape model parameters to not deviate more the 3 standart deviations
          max_b = 3*math.sqrt(self.asm.evals[i])#/TOTALLEVELS**scale #scale represents the level
          b = max(min(b, max_b), -max_b)
          #use parameters to calculate new shape model
          new = new + self.asm.evecs[:,j]*b
          break
    '''
