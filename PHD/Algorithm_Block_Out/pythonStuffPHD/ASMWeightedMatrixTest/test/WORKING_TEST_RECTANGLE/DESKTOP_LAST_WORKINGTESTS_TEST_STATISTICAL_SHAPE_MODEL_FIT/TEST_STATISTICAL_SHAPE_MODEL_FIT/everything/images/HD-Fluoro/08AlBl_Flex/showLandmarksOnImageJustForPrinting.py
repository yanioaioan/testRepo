...
def show_my_shapes(shapes, initShapes):
  ...
  for i in range(len(initShapes)):#was len(shapes)
    ...
    imageCounterColor = int(sys.argv[2].split('_')[-1].split('.')[0])
...
class PointsReader ( object ):
  ...
  def read_directory(dirname):
    ...
    for file in glob.glob(os.path.join(dirname, "*.pts")):
      #print file
      fileNo = (file.split('/')[-1]).split('.')[0].split('_')[-1]
      try:
         fileNo = int(fileNo)
      except ValueError:
          print("That's not a simple int!")
          print fileNo
          fileNo = fileNo.split('_')[0]
...
shapes,filesReadOrder = PointsReader.read_directory(sys.argv[1])
initShapes = 0
if len(sys.argv) > 3 :
    initShapes,filesInitShapesReadOrder = PointsReader.read_directory(sys.argv[3])
    print sys.argv[3]

print filesReadOrder
print filesInitShapesReadOrder
...
show_my_shapes(shapes, initShapes)
...
for shape in shapes:
    ...
    for i,p in enumerate(shape.pts):
        ...
        nextPoint = ( int(tmpP.x), int(tmpP.y) )
        if prevPoint != 0:
          cv.Line(targetimg, prevPoint , nextPoint ,(0,0,255),1)
        prevPoint = ( int(tmpP.x), int(tmpP.y) )
    ...
    cv.SaveImage("exportedAfterSegmentationStatic/LastExport/exportedFrame_"+str(imageCounterColor)+"_.jpg", targetimg)
