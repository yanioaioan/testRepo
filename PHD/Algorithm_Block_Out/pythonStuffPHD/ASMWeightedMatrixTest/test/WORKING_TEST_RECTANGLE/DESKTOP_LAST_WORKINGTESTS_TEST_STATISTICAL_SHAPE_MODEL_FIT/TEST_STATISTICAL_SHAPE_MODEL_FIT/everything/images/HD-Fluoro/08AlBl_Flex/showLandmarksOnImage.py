...
def show_my_shapes(shapes, initShapes):
  ...
  for i in range(len(initShapes)):#was len(shapes)
    ...
    for i in range(len(path)-1):
        imagePath+=path[i]+"/"
	print imagePath
	imagenumber = path[-1].split('.')[0].split('_')[-1]
	
	#imageCounterColor+=1
	imageCounterColor = int(imagenumber)
    ...
    cv.NamedWindow("landmarks On Target Image %sgrey_image_%d.jpg"%(imagePath,imageCounterColor), cv.CV_WINDOW_NORMAL)
    cv.ShowImage("landmarks On Target Image %sgrey_image_%d.jpg"%(imagePath,imageCounterColor), i)
    cv.SaveImage("exportedAfterSegmentationOverTime50frames/exportedFrame_"+str(imageCounterColor)+"_.jpg", i)
...
initShapes = 0
if len(sys.argv) > 3 :
    initShapes = PointsReader.read_directory(sys.argv[3])
    print sys.argv[3]
...
for shape in shapes:
    ...
    cv.SaveImage("exportedAfterSegmentationOverTime50frames/exportedFrame_"+str(imageCounterColor)+"_.jpg", targetimg)
cv.WaitKey()
'''
