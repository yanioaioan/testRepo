...
def gradient(img, dx, dy, ksize):
    ...
    cv2.imwrite("tmpImagetoGrab1stDerivativeFrom.jpg",img,)

    img = cv2.imread('tmpImagetoGrab1stDerivativeFrom.jpg')
...
class FitModel:
  ...
  def __init__(self, asm, image, targetImageName, originalShapes, t=Point(0,0)):#x,y #5,85 for c3
...
#######cv.WaitKey()
  ...
  def calculateIthImageLandmarkDerivIntensityVec(self, p, norm, greyImage, gaussianMatrix, _searchProfiles, IthImage_NormalizedtmpLandmarkDerivativeIntensityVector, gaussianWindow):
      ...
      IthImage_NormalizedtmpLandmarkDerivativeIntensityVector.append(yij_normalizedDerivProfile[0])
  ...
  def findLandmarkNormIIntesityVec(self, gMeanFlag, oneTrainingImageVector, shape, trainingSetImage, IthImage_NormalizedtmpLandmarkDerivativeIntensityVector,gaussianMatrix, greyImage):
      ...
      for i,p in enumerate(shape.pts):
          ...
          if gMeanFlag==1:
              ...
              tmpP = Point(p.x, p.y)
              cv.Circle(self.image, ( int(tmpP.x), int(tmpP.y) ), 1, (i*10,i*20,i*30))
              cv.NamedWindow("targetImage", cv.CV_WINDOW_NORMAL)
              cv.ShowImage("targetImage",self.image)
              ####cv.WaitKey()
  ...
  def findtrainingGmean(self, testImageNameCounter, AllImages_NormalizedtmpLandmarkDerivativeIntensityVector, gaussianMatrix):
      ...
      for imagelevel in range(self.pyramidLevels):
          ...
          for asmshape in self.originalShapes:#self.asm.shapes !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 3 for now
              ...
              gradimg=gradient(gradimg,1,1,5)
          ...
          '''   AllImages_NormalizedtmpLandmarkDerivativeIntensityVector holds 4 shapes x 20 points x 6 profile x 13 elements each  '''
...
#                  #######cv.WaitKey()
      ...
      #####print"\n np.array(y_j_mean)=%s"%(np.array(y_j_mean[0]).flatten())
          ...
          #m from 0<=m<=5
          #k 3 pixels on each side of the normal
          eithersideOfNormalPixels=3
          side= 3*(bestIndexProfile-eithersideOfNormalPixels)+1 # (-5..-3..-1...1...3..5..)  , #bestIndexProfile-3


          print "shapes length = %d"%(x.shape[0])
          print "bestIndexProfile is %d..so final chosen side is: %d"%(bestIndexProfile,side)
          cv.WaitKey(1)
          ...
          if (bestIndexProfile < (halfShapeProfile )) and self.currentSearchPyramidLevel >=1:#as it can't go further down than 0 level in res pyramid
              ...
              #A convergence criterion is that a sufficient number of landmarks are reached (in this case we choose half of the total number of landmarks)
          ...
          #SHOW ALL POSSIBLE CHOICE BASED ON MAHALANOBIS WITH BLUE COLOR plus THE ACTUAL CHOIC WITH GREEN COLOR
          ...
          for j in range(x.shape[0]):#for all the profile elements
              ...
              mypoint = Point(p.x + (2*(j-eithersideOfNormalPixels)+1)*norm[0], p.y + (2*(j-eithersideOfNormalPixels)+1)*norm[1])


              if ( int(new_p.x)==int(mypoint.x) and int(new_p.y)==int(mypoint.y) ):
                  cv.Circle(TESTCHOICE, ( int(mypoint.x), int(mypoint.y) ), 1, (0,255,0))
              else:
                  cv.Circle(TESTCHOICE, ( int(mypoint.x), int(mypoint.y) ), 1, (255,0,0))
          ...
          if targetImageTocheckAgainst[int(new_p.y), int(new_p.x)] > max_edge:#this is likely to represent an edge (as targetImageTocheckAgainst contains the 1st derivative of the actual image, smoothed for thislevel)

            max_edge = targetImageTocheckAgainst[int(new_p.y), int(new_p.x)] #greyImage[scale][y, x]
            max_pt = Point(int(new_p.x) , int(new_p.y)) #x,y   #new_p.x + t*norm[0], new_p.y + t*norm[1]
