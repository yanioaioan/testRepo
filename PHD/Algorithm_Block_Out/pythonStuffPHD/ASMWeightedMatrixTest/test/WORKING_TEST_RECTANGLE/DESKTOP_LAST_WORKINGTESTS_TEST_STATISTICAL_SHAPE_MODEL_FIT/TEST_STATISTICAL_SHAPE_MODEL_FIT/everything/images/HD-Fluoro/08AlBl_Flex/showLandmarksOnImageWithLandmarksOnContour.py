...
def show_my_shapes(shapes, initShapes, filesReadOrderExportedShapes, filesReadOrderInitShapes):

  """ Function to show all of the shapes which are passed to it
  """

  global imageCounterColor


  for elem in range(len(initShapes)):#was len(shapes)

    shape = shapes[elem]
    shape2 = 0
    if len(sys.argv) > 3 :
        shape2 = initShapes[elem]

    #if it doesn't end with '/', then append one
    path=''
    if not sys.argv[2][-1] == '/':
      path = sys.argv[2] +str('/')
      print 'fixing path by appendin \"/"'
    ...
    #imageCounterColor+=1
    #print filesReadOrderExportedShapes
    print 'filesReadOrderExportedShapes[elem]=',filesReadOrderExportedShapes[elem]


    imageCounterColor=int(filesReadOrderExportedShapes[elem])

    #to fix trying to grab grey_image_0.jpg


    print "drawing points of %d.pts on %sgrey_image_%d.jpg"%(imageCounterColor,imagePath,imageCounterColor)

    i = cv.LoadImage("%sgrey_image_%d.jpg"%(imagePath,imageCounterColor))#reload base image every time (for every shape), to have all pts drawn on the same image (to be able to compare the non-shakiness of this and the 'over time' computation)

    '''show shape variation on black background'''
    #i = cv.LoadImage("testBackground.png")

    print 'draw shape lines'

    #Final Shape
    nextPoint=-1
    prevPoint=-1

    for pt_num, pt in enumerate(shape.pts):#draw this shape's contour

      nextPoint = (int(pt.x), int(pt.y))
      if prevPoint != -1:
          #cv.Line(i, prevPoint , nextPoint ,(0,255,255),1)
          cv.Circle(i, (int(nextPoint[0]), int(nextPoint[1])), 2, (0, 0, 1), -1)
          cv.NamedWindow("landmarks On Target Image %sgrey_image_%d.jpg"%(imagePath,imageCounterColor), cv.CV_WINDOW_NORMAL)
          cv.ShowImage("landmarks On Target Image %sgrey_image_%d.jpg"%(imagePath,imageCounterColor), i)

      prevPoint = nextPoint

    #connect second-last back to last/first point
    cv.Line(i, prevPoint , (int(shape.pts[0].x), int(shape.pts[0].y)) ,(0,255,255),1)
    cv.NamedWindow("landmarks On Target Image %sgrey_image_%d.jpg"%(imagePath,imageCounterColor), cv.CV_WINDOW_NORMAL)
    cv.ShowImage("landmarks On Target Image %sgrey_image_%d.jpg"%(imagePath,imageCounterColor), i)
    cv.SaveImage("exportedAfterSegmentationStatic/exportedFrame_"+str(imageCounterColor)+"_.jpg", i)

    #Initial Shape
    print len(sys.argv)
    if len(sys.argv) > 3 :
      #print 'enter path to initShapes as a 3rd argument too!!'

      #Initial Shape
      imageCounterColor=int(filesReadOrderInitShapes[elem])
      for pt_num, pt in enumerate(shape2.pts):#draw this shape's contour
      ...
      #connect second-last back to last/first point
      cv.Line(i, prevPoint , (int(shape2.pts[0].x), int(shape2.pts[0].y)) ,(0,0,255),1)
      cv.NamedWindow("landmarks On Target Image %sgrey_image_%d.jpg"%(imagePath,imageCounterColor), cv.CV_WINDOW_NORMAL)
      cv.ShowImage("landmarks On Target Image %sgrey_image_%d.jpg"%(imagePath,imageCounterColor), i)
      cv.SaveImage("exportedAfterSegmentationStatic/exportedFrame_"+str(imageCounterColor)+"_.jpg", i)

  #c= cv.WaitKey(0)#press any key to exit


class PointsReader ( object ):
  """ Class to read from files provided on Tim Cootes's website."""
  @staticmethod
  def read_points_file(filename,fileNo=0):
  ...
  def read_directory(dirname):
    ...
    pts = []
    filesReadOrder = []
    #print glob.glob(os.path.join(dirname, "*.pts"))
    ...
    for file in sorted(glob.glob(os.path.join(dirname, "*.pts")), key=os.path.getmtime):
      ...
      #if not a number (pts name contains more than a simple int such as '1_frames__scale_0_frame_1_range-51-75_.pts'
      #throw ..
      try:
         fileNo = int(fileNo)
      except ValueError:
          print("That's not a simple int!")
          fileNo = fileNo.split('_')[-1]
          print 'fileNo',fileNo
    ...
    print 'filesReadOrder=',filesReadOrder
    return pts,filesReadOrder

filesReadOrderInitShapes=0
filesReadOrderExportedShapes=1

exportedShapes,filesReadOrderExportedShapes = PointsReader.read_directory(sys.argv[1])

initShapes = 0
if len(sys.argv) > 3 :
    initShapes,filesReadOrderInitShapes = PointsReader.read_directory(sys.argv[3])
    print sys.argv[3]


print "initShapes",len(initShapes)
print "shapes",len(exportedShapes)
#cv.WaitKey()

print filesReadOrderExportedShapes
print filesReadOrderInitShapes

#uncomment the following to..
#draw segmented lines/contours
show_my_shapes(exportedShapes, initShapes,filesReadOrderExportedShapes, filesReadOrderInitShapes)
...
for shape in shapes:
    ...
    #targetimg = cv.LoadImage(sys.argv[2])#reload base image every time (for every shape), to have all pts drawn on the same image (to be able to compare the non-shakiness of this and the 'over time' computation)
    targetimg = cv.LoadImage("%sgrey_image_%d.jpg"%(imagePath,imageCounterColor))#reload base image every time (for every shape), to have all pts drawn on the same image (to be able to compare the non-shakiness of this and the 'over time' computation)
    ...
    for i,p in enumerate(shape.pts):
        ...
        #cv.WaitKey(0)
        cv.NamedWindow("landmarks On Target Image %sgrey_image_%d.jpg"%(imagePath,imageCounterColor), cv.CV_WINDOW_NORMAL)
        cv.ShowImage("landmarks On Target Image %sgrey_image_%d.jpg"%(imagePath,imageCounterColor), targetimg)
    #cv2.imwrite("exportedAfterSegmentation/exportedFrame_"+i+"_.jpg", image)
    #cv.SaveImage("exportedAfterSegmentationStatic/exportedFrame_"+str(imageCounterColor)+"_.jpg", targetimg)
    cv.SaveImage("exportedAfterSegmentationStatic/exportedFrame_"+str(imageCounterColor)+"_.jpg", targetimg)
cv.WaitKey()
'''
