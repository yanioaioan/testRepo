...
ENHANCE_IMAGE=False#increase contrast & make vertebras differer from background
...
#STEP=5#sample every "STEP" pixels along the whisker # was 10 WORKS FOR FRANK-->6 works for fluoroTest
#Normal Profile range on each side
#PIXELS_SEARCH=6 #COULD BE 2 # was 4    #6 work with   __createStatisticalProfileModel() & and self.__get_MAHALANOBIS()



#How many extra pixels in each direction
STEP=7#sample every "STEP" pixels along the whisker # was 10 WORKS FOR FRANK-->6 works for fluoroTest

#Normal Profile range on each side
PIXELS_SEARCH=8 #COULD BE 2 # was 4    #6 work with   __createStatisticalProfileModel() & and self.__get_MAHALANOBIS()
...
BRIGHTNESS_DECREASE_COEF = 50
CONTRAST_DECREASE_COEF = 1.5
...
class Shape ( object ):
  ...
  def add_point(self, p, degreesOfRotation=0, CenterOfMass=Point(0,0)):
      ...
      #print "degreesOfRotation=",degreesOfRotation
      ...
      #ox, oy = CenterOfMass[0],CenterOfMass[1]
      #print "CenterOfMass=",CenterOfMass
  ...
  def add_point_prev(self, p, degreesOfRotation=0, CenterOfMass=Point(0,0)):
    ...
    if CenterOfMass.x != 0 and CenterOfMass.y != 0 and degreesOfRotation!=0:
        #degreesOfRotation=-15
        f=degreesOfRotation*(math.pi/180.0)
        ...
        '''
        print "centerOfMass=",CenterOfMass
        print "degreesOfRotation=",f

        #https://stackoverflow.com/questions/10533403/how-to-rotate-a-polygon-around-a-point-with-java          rflow.com/questions/10533403/how-to-rotate-a-polygon-around-a-point-with-java
        #http://forums.codeguru.com/showthread.php?346093-Rotate-polygon                                        deguru.com/showthread.php?346093-Rotate-polygon
        p.x = CenterOfMass.x + ((p.x-CenterOfMass.x)*math.cos(f)-(p.y-CenterOfMass.y)*math.sin(f))              #s.x + ((p.x-CenterOfMass.x)*math.cos(f)-(p.y-CenterOfMass.y)*math.sin(f))
        p.y = CenterOfMass.y + ((p.x-CenterOfMass.x)*math.sin(f)+(p.y-CenterOfMass.y)*math.cos(f))              #s.y + ((p.x-CenterOfMass.x)*math.sin(f)+(p.y-CenterOfMass.y)*math.cos(f))
...
meanshapeVec = Shape([])
frame_shape_counter=1
finalPlacedShapeList = []
...
class ShapeViewer ( object ):
  ...
  @staticmethod
  def draw_model_fitter(f,scale,iterationIndex):
    ...
    for frame in range (len(f.target_images)):#Draw All shape models initialized on all frames to start fitting
      scaleChosen=0#use the 1st unsmoothed images, to place the mean shape as accurately as possible!

      #Adjustment Trails
      if TRAILS:
        '''
        imagelist.append(cv.CreateImage(cv.GetSize(f.g_images[frame*TOTALLEVELS+scaleChosen]), f.g_images[frame*TOTALLEVELS+scaleChosen].depth, 1))
        cv.Copy(f.g_images[frame*TOTALLEVELS+scaleChosen], imagelist[frame])
        '''


        convertedToColouredImg = cv.CreateImage(cv.GetSize( f.g_target_images[frame*TOTALLEVELS+scaleChosen]), cv.IPL_DEPTH_8U, 3)
        cv.CvtColor(f.g_target_images[frame*TOTALLEVELS+scaleChosen], convertedToColouredImg, cv.CV_GRAY2BGR)

        imagelist.append(convertedToColouredImg)

        #convertedToColouredImg = cv.CreateImage(cv.GetSize( f.g_images[frame*TOTALLEVELS+scaleChosen]), cv.IPL_DEPTH_8U, 3)
        #cv.CvtColor(f.g_target_image[scaleChosen], convertedToColouredImg, cv.CV_GRAY2BGR)
        #Shows original target
        cv.Copy(convertedToColouredImg, imagelist[frame])




      ##Shows derivatives of target image
      #convertedToColouredImg = cv.CreateImage(cv.GetSize( f.g_target_image[scaleChosen]), cv.IPL_DEPTH_8U, 3)
      #cv.CvtColor(f.g_target_image[scaleChosen], convertedToColouredImg, cv.CV_GRAY2BGR)
      ...
      #change the model fitter colour based on the current level of image search
      levelColour=(0,0,255)
      if scale==0:
          levelColour=(0,0,255)#lowest level
      elif scale==1:
          levelColour=(0,255,0)
      elif scale==2:
          levelColour=(255,0,0)#highest level


      #for currentShape in f.allInitializedShapes:#Draw All shape models initialized on all frames to start fitting

      #needed when there are no TRAILS enabled
      if len(imagelist)==0:
      ...
      for pt_num, pt in enumerate(f.allInitializedShapes[frame].pts):
        # Draw normals

        #cv.Circle(imagelist[frame], (int(pt.x), int(pt.y)), 1, (100-iterationIndex*50,iterationIndex*50,255-iterationIndex*50), -1)#Un Hash to have points

        print len(imagelist)
        #cv.WaitKey()

        cv.Circle(imagelist[frame], (int(pt.x), int(pt.y)), 1, levelColour, -1)#Un Hash to have points
        nextPoint = (int(pt.x), int(pt.y))

        if prevPoint != -1:#draw line connecting every other point with the next one , except for the last to the first segment
            #cv.Line(imagelist[frame], prevPoint , nextPoint ,(0,0,255),1)
            print 1

        else:#draw line segment connecting the last to the first landmark
            prevPoint = (int(f.allInitializedShapes[frame].pts[-1].x), int(f.allInitializedShapes[frame].pts[-1].y))
            #cv.Line(imagelist[frame], prevPoint , nextPoint ,(0,0,255),1)

        #print 'model fitter %d'%(frame)

        prevPoint = nextPoint
        cv.NamedWindow("Model Fitter frame %d"%(frame), cv.CV_WINDOW_NORMAL)
        cv.ShowImage("Model Fitter frame %d"%(frame),imagelist[frame])
...
class PointsReader ( object ):
  ...
  def read_directory(dirname):
    ...
    pts = []
    filesReadOrder = []
    for file in sorted(glob.glob(os.path.join(dirname, "*.pts")), key=os.path.getmtime):
      fileNo = (file.split('/')[-1]).split('.')[0]
      filesReadOrder.append(fileNo)
      pts.append(PointsReader.read_points_file(file))
    return pts,filesReadOrder
...
class FitModel:
  ...
  pointsOffsetsDict = {}#point offset dictionary
  #framePointsOffsets = []#this iteration's and this frame's points' offset/displacement
  ...
  def pyramid(self, image, level, imagenumber=0, totaleLevel=0):
      ...
      if level:# downsample and upsample only if level not zero
  ...
  def __init__(self, asm, images, imageNames, target,    target_images ,target_imageNames  ,originalShapes, answer='',  t=initPoint):#round shape
  ...
  #def __init__(self, asm, image, t=Point(140,45.0)):
    ...
    #all the shapes initialized manually now, one by one (these are about to be extracted and assigned to self.shape at the time of get_mahalanobis distance)
    self.allInitializedShapes = []
    ...
    for i in range(0,TOTALLEVELS):#for 4 different levels of sobel derivative smoothing
      if not USE_PYRAMID_SMOOTHING:
        ...
        if i==0:
          ...
          currentSmoothedLevelTarget=self.pyramid(sys.argv[2],i)
          print "testPyr_%s.jpg"%(i)
          print 'xoox'
        else:
          print "testPyr_%s.jpg"%(i-1)
          print 'xiix'
          currentSmoothedLevelTarget=self.pyramid("testPyr_%s.jpg"%(i-1),i)
        ...
        for i in range(0,TOTALLEVELS):#for 4 different levels of sobel derivative smoothing
              ...
              if not USE_PYRAMID_SMOOTHING:
                ...
                if i==0:
                  ...
                  currentSmoothedLevelTarget=self.pyramid(imageNames[imagenumber],i,  imagenumber, i)
                else:
                  #currentSmoothedLevelTarget=self.pyramid("testPyr_%s.jpg"%(1),i+1)
                  currentSmoothedLevelTarget=self.pyramid("testPyr_%s.jpg"%(i-1),i)
    ...
    #now smooth the target images
    for imagenumber in range(0,len(target_images),1):#for each image
        for i in range(0,TOTALLEVELS):#for 4 different levels of sobel derivative smoothing
              ...
              if not USE_PYRAMID_SMOOTHING:
                self.g_target_images.append(self.__produce_gradient_image(target_images[imagenumber], 2**i))
              else:

                if i==0:
                  #imageNames[imagenumber] is the current training imag name
                  #currentSmoothedLevelTarget=self.pyramid(imageNames[imagenumber],i+1,  imagenumber, i)
                  currentSmoothedLevelTarget=self.pyramid(target_imageNames[imagenumber],i,  imagenumber, i)
                else:
                  #currentSmoothedLevelTarget=self.pyramid("testPyr_%s.jpg"%(1),i+1)
                  currentSmoothedLevelTarget=self.pyramid("testPyr_%s.jpg"%(i-1),i)
                ...
                self.g_target_images.append(result_sobelTestPyr)
    ...
    print "Reading..ShapeFrame%d.pts, instead of reproducing"%(frame_shape_counter)
    #global answer
    #answer = raw_input("where to read from under initShapes ??")
    #initialize  self.allInitializedShapes with what's been read from initShapes dir
    cwd = os.getcwd()

    #Initialize manually every frame only if there's no shape information found under initShapes
    if len(os.listdir((str(cwd) +str('/') + str(self.answer))))==0:


        #waiting all frames to finish manual initialization, for iteration N
        frame=0
        while frame<len(images):
          ...
          while True:
              ...
              scale=0#use the 1st unsmoothed images, to place the mean shape as accurately as possible!

              #use each equivalent image frame, to initialize!!
              targetCopy = cv.CreateImage(cv.GetSize(self.g_target_images[frame*TOTALLEVELS+scale]), self.target.depth, 1)
              cv.Copy(self.g_target_images[frame*TOTALLEVELS+scale], targetCopy)
              ...
              if key == ord('e'):
                  ...
                  print 'settled for this shape, move on to the next one now..'
                  cv.WaitKey()

                  finalPlacedShapeList.append(finalPlacedShape)
          ...
          frame+=1#fetch the next frame of the video sequence to initialize the mean shape onto
          cv.WaitKey()
          ...
          tmpShape = Shape.from_vector(finalPlacedVec)#.transform(t, degreesOfRotation, CenterOfMass)


          # And resize shape to fit image if required
          if self.__shape_outside_image(tmpShape, self.images[0]):
            tmpShape = self.__resize_shape_to_fit_image(self.shape, self.images[0])

          self.allInitializedShapes.append(tmpShape)#.transform(t, degreesOfRotation, CenterOfMass))
          print self.allInitializedShapes

        print 'All mean shapes initialized on all video sequence frames!, allInitializedShapes should now contain all initialized shapes to work with during get_mahalanobis in parallel'

        print 'number of allInitializedShapes=',len(self.allInitializedShapes)
        cv.WaitKey()



    #write out the init shape instead so as to avoid reinitializing them everytime
    global frame_shape_counter
    if len(os.listdir((str(cwd) +str('/') + str(self.answer))))==0:#if initShapes dir empty
      if not os.path.exists( "initShapes/"+stringfileName+"_"+str(STEP)+"_"+str(PIXELS_SEARCH)+"ShapeFrame%d.pts"%(frame_shape_counter) ):



        for initShape in finalPlacedShapeList:
          with open("initShapes/"+stringfileName+"_"+str(STEP)+"_"+str(PIXELS_SEARCH)+"ShapeFrame%d.pts"%(frame_shape_counter), 'w') as initShapesFile:

            print "writing to.."
            print "initShapes/"+stringfileName+"_"+str(STEP)+"_"+str(PIXELS_SEARCH)+"ShapeFrame%d.pts"%(frame_shape_counter)

            initShapesFile.write("version: 1\n")
            initShapesFile.write("n_points: %d\n"%( len(self.allInitializedShapes[0].pts) ) )
            initShapesFile.write("{\n")

            frame_shape_counter+=1
            for i, pt in enumerate (initShape.pts):
                print pt.x, pt.y


                #write init shapes out, 1 by 1

                initShapesFile.write(str(int(pt.x)))
                initShapesFile.write(" ")
                initShapesFile.write(str(int(pt.y)))#init shape written out
                initShapesFile.write("\n")

            print "And finally, we close it."
            initShapesFile.write("}\n")
            initShapesFile.close()




    else:#if any of the init shape found


        #shapes = PointsReader.read_directory(str(cwd)+'/initShapes')
        print 'read from --> %s'%( (str(cwd) +str('/') + str(self.answer)) )
        shapes,filesReadOrder = PointsReader.read_directory(str(cwd) +str('/') + str(self.answer))

        frame=0
        for shape in shapes:

            finalPlacedVec = shape.get_vector()
            #self.shape = Shape.from_vector(asm.mean).transform(t, degreesOfRotation, CenterOfMass)
            ...
            #use each equivalent image frame, to initialize!!
            print 'len(g_target_images) = ',len(self.g_target_images)
            targetCopy = cv.CreateImage(cv.GetSize(self.g_target_images[frame*TOTALLEVELS+scale]), self.target.depth, 1)
            cv.Copy(self.g_target_images[frame*TOTALLEVELS+scale], targetCopy)
            ...
            drawMeanShape(shape.get_vector(), targetCopy)

            cv.NamedWindow("targetCopyyyyyyy", cv.CV_WINDOW_NORMAL)
            cv.ShowImage("targetCopyyyyyyy",targetCopy)

            print 'read and placed another one of the initialized shapes under initShapes'
            frame+=1#fetch the next frame of the video sequence to initialize the mean shape onto
    ...
    if ENHANCE_IMAGE:
        for i in range(grey_image.height):
            for j in range(grey_image.width):
                ...
                grey_image[i,j] = grey_image[i,j] - BRIGHTNESS_DECREASE_COEF
                #print 'increasing contrast'
                grey_image[i,j] = grey_image[i,j] * CONTRAST_DECREASE_COEF
  ...
  def do_iteration(self, scale, reset_giAllPointsShapeVecSet, iterationIndex, frame, imagesRange):
    ...
    if USE_MAX_EDGE:
        ...
        if reset_giAllPointsShapeVecSet==1:
            ...
            print "trainingSetProfiles/"+stringfileName+"_"+str(STEP)+"_"+str(PIXELS_SEARCH)+"_trainingSetLevel_%d_range_%s.txt"%(scale,imagesRange)
            #cv.WaitKey()
            if not os.path.exists( "trainingSetProfiles/"+stringfileName+"_"+str(STEP)+"_"+str(PIXELS_SEARCH)+"_trainingSetLevel_%d_range_%s.txt"%(scale,imagesRange) ):
                with open("trainingSetProfiles/"+stringfileName+"_"+str(STEP)+"_"+str(PIXELS_SEARCH)+"_trainingSetLevel_%d_range_%s.txt"%(scale,imagesRange), 'w') as f:#write out the writeOutStatisticalTrainingProfileModel
                    ...
                    for i in range(len(self.originalShapes[0].pts)):#for each point of any shape (we used originalShapes[0] to get the length of the list containing the landmarks)
                        ...
                        for imageShapenumber in range(0,len(self.images),1):#for each training image/shape
                            ...
                            cv.SaveImage("SAMPLING TEST-%s.jpg"%(imagesRange), self.g_images[imageShapenumber*TOTALLEVELS+scale])
                            ...
                            path = 'SAMPLING TEST-%s.jpg'%(imagesRange)
                ...
                print "Reading..trainingSetLevel_%d_range_%s.txt, instead of reproducing"%(scale,imagesRange)
                #cv.WaitKey()
                print "trainingSetProfiles/"+stringfileName+"_"+str(STEP)+"_"+str(PIXELS_SEARCH)+"_trainingSetLevel_%d_range_%s.txt"%(scale,imagesRange)
                #cv.WaitKey()
                with open("trainingSetProfiles/"+stringfileName+"_"+str(STEP)+"_"+str(PIXELS_SEARCH)+"_trainingSetLevel_%d_range_%s.txt"%(scale,imagesRange), 'r') as f:
                    lines = f.readlines()
                ...
                for line in lines:
                    ...
                    print len(self.originalShapes[0].pts)
                    print '1615',len(lines)
    ...
    for i, pt in enumerate(self.allInitializedShapes[frame].pts):#save each frame's shape displacements pointsOffsets as a list
      ...
      if USE_PROFILE_MODEL:
        if SAMPLING_PY_ENABLED:
            #get the displacements for all images at this level (instead of just one), so as to average them in the following step

            #for i in range(len(self.originalShapes[0].pts)):#for each point of any shape (we used originalShapes[0] to get the length of the list containing the landmarks)
            #for imageShapenumber in range(0,len(self.images),1):#for each training image/shape

            cv.NamedWindow("SAMPLING TEST Statistical model sampling", cv.CV_WINDOW_NORMAL)
            cv.ShowImage("SAMPLING TEST Statistical model sampling", self.g_images[frame*TOTALLEVELS+scale])
            ...
            point, savedOffset = self.sampling.get_MAHALANOBIS(self.allInitializedShapes[frame], self.g_target_images[frame*TOTALLEVELS+scale], TrainingCovarianceMatrices, TrainingMean, i, scale, createImageOncePerIteration, frame)

            #s.add_point(point)#, curBest

            #save offsets
            tmpPointOffsetList.append(savedOffset)
            print 'tmpPointOffsetList=%r'%(tmpPointOffsetList)
            #cv.WaitKey()
    ...
    framePointsOffsets.append(tmpPointOffsetList)#saved 41 displacements

    #clear convertedToColouredImg variable from memory
    cv.DestroyWindow("convertedToColouredImgTMP_%d"%(frame))

    #testAveragedDisplacementsListForThisFrame=map(multipleListsMean, zip(*pointsOffsets))
    #data = np.array(framePointsOffsets)
    #print data
    #cv.WaitKey()
    #testAveragedDisplacementsListForThisFrame=np.average(data, axis=0)

    self.pointsOffsetsDict[frame] = framePointsOffsets#saving pointOffsets of this frame
    print("self.pointsOffsetsDict for frame=%r %r\n")%(self.pointsOffsetsDict,frame)
    print 'frame=',frame
    print 'self.g_target_images)-1=',len(self.target_images)-1

    while frame<len(self.target_images)-1:#MAYBE GO BECK TO len(self.images)
        print "waiting all frames to finish for iteration %d\n"%(iterationIndex)
        #cv.WaitKey()
        return


    #When all frames have finished, do the averaging of their displacements, pointwise
    data = np.array( self.pointsOffsetsDict.values())
    # ..                                                                axis=0 means to average vertically
    testAveragedDisplacementsListForThisFrame= np.rint( np.average(data, axis=0) )#average per point displacements to displace all initialized shapes
    print 'testAveragedDisplacementsListFor frame %d ThisFrame %r'%(frame,testAveragedDisplacementsListForThisFrame)
    #cv.WaitKey()
    ...
    m = stats.mode(data)
    mode=m[0]#mode is the frequency of the column values (the one that occurs more frequently is returnt)
    counts=m[1]#The count is telling us how many times it has seen the reported mode in each of the columns
    print mode
    print counts

    #OVERIDE PREVIOUS testAveragedDisplacementsListForThisFrame with 'mode' calculation instead of 'average'
    testAveragedDisplacementsListForThisFrame = (mode[0])
    #VOTING THE MOST FREQUENTLY SELECTED DISPLACEMENT ACROSS ALL TESTED FRAMES, SO AS TO DISPLACE ALL SHAPES TO..!
    #THIS WAY VARIANT NOISE ON CERTAIN FRAMES IS BEING HANDLED COMMONLY BASED ON AVERAGE PIXEL INFORMATION ACROSS FRAMES, PREVENTING WRONG SEGMENTATION


    print np.array(mode[0])
    #cv.WaitKey()
    print 'mode[0]',(mode[0])
    #cv.WaitKey()
    ...
    #Now I need to transpose the columns of data into rows to feed the correctly to 'testdataset'
    landmarkDisplacementsAccrossRange = np.transpose(data)
    print 'landmarkDisplacementsAccrossRange',landmarkDisplacementsAccrossRange

    #Now append as the last column to landmarkDisplacementsAccrossRange mode[0], mode[0] needs to be the final column accross all rows in testdataset.csv
    #landmarkDisplacementsAccrossRange= np.append(landmarkDisplacementsAccrossRange, mode[0])

    landmarkDisplacementsAccrossRange=np.squeeze(landmarkDisplacementsAccrossRange.tolist())

    print 'a',landmarkDisplacementsAccrossRange
    print 'b',mode[0]


    modeSelected=[]
    mode[0]= mode[0].flatten()
    for x in np.nditer(mode[0]):
         modeSelected.append([int("%d"%(x))])

    modeSelected = np.array(modeSelected)

    print 'a',landmarkDisplacementsAccrossRange
    print 'modeSelected',modeSelected

    decisionTreeOfThisRange=np.append(landmarkDisplacementsAccrossRange, modeSelected, axis=1)

    #CLEAR FOREST START temp decision trees, forset prediction of the previous iteration and landmarks observations for this iteration from which we formed the previous iteration's prediction
    #remove decision tree of this range, to have the next iteration in..
    if (os.path.isfile("DecisionTrees/decisionTreeOf-%s-Range_%d.txt"%(imagesRange,iterationIndex) )):
      #os.remove("DecisionTrees/decisionTreeOf-%s-Range_%d.txt"%(imagesRange,iterationIndex) )
      print 'tmp removing is remove'
    #remove forestPrediction.txt of this iteration, to have the next iteration in..
    if (os.path.isfile("DecisionTrees/forestPrediction_%s_%d.txt"%(imagesRange,iterationIndex))):
      #os.remove("DecisionTrees/forestPrediction_%s_%d.txt"%(imagesRange,iterationIndex))
      print 'TEMPORARILY DON@T REMOVE',"DecisionTrees/forestPrediction_%s_%d.txt"%(imagesRange,iterationIndex)
    #if any of the .csv files are there, then remove all .csv, to have the next iteration in..
    decision_dir_name = "DecisionTrees/individualLandmarkForests/"
    dir = os.listdir(decision_dir_name)
    for file in dir:
        if file.endswith("%s_%s.csv"%(iterationIndex-1,imagesRange)):#delete the previous iteration's csv files

          #if file.endswith("*_%d.csv"%(iterationIndex)):
          try:
            os.remove(os.path.join(decision_dir_name, file))
            print 'removing ',file
          except:
             print 'no worries %s already removed'%(file)
    #CLEAR FOREST END temp

    print 'landmarkDisplacementsAccrossRange + mode --> decisionTreeOfThisRange : ',decisionTreeOfThisRange
    decisionTreeOfThisRange=decisionTreeOfThisRange.tolist()

    print imagesRange
    f=open("DecisionTrees/decisionTreeOf-%s-Range_%d.txt"%(imagesRange,iterationIndex),'w')
    counter=0
    for ele in decisionTreeOfThisRange:
        counter+=1
        print "%r, %r\n"%(counter,ele)
        f.write("%r\n"%ele)
    ...
    '''
    #write out 'decisionTreeOfThisRange'
    with open("DecisionTrees/decisionTreeOf-%s-Range.txt"%(imagesRange), "w") as f:
        for item in decisionTreeOfThisRange:
            f.write("%s\n" % item)
    f.close()
    '''


    ### Wait and check till if a specific file containing the prediction out of the forest calculation is there.
    ### This suggests that the rest of the investigated ranges
    ### 1) have reach this stage, 2)have exported their 'decisionTreeOfThisRange', 3)a prediction has been written
    ### for all executed ranges to read from and continue
    #while not (os.path.isfile('./DecisionTrees/forestPrediction_%s_%d.txt'%(imagesRange,iterationIndex))):

    #Call python readCurrentIterationAccrossRangeForXLandmarkAndAppendToForest.py, only when none of the landmardk csv files are there.
    import glob
    pathToDecisionTrees=os.getcwd()+str('/DecisionTrees/')
    pathToRandomForest='/transfer/myphd_TowardsTransferInparallelFrames/RandomForestTests'

    #if not glob.glob('./DecisionTrees/individualLandmarkForests/testDataset_landmark_%d_forest_%d.csv'%(counter,iterationIndex)):      #if not (os.path.isfile('./DecisionTrees/testDataset_landmark_1_forest.csv')):
    #if any of the testDataset_landmark_#_forest.csv files are there means all have been exported from a previously run instance of 'testInitASM.py', and that there's NO need to reexport 'em!!
    sys.path.append('./DecisionTrees')
    sys.path.append(pathToRandomForest)# /home/yioannidis/Desktop/myphd_29_1_2017_continueINPARALLELFRAMES/RandomForestTests
    #import readCurrentIterationAccrossRangeForXLandmarkAndAppendToForest
    os.system('python DecisionTrees/readCurrentIterationAccrossRangeForXLandmarkAndAppendToForest.py %s %s'%(imagesRange,iterationIndex))

    arg="decisionTreeOf-%s-Range_%d.txt"%(imagesRange,iterationIndex)
    #os.system('python readCurrentIterationAccrossRangeForXLandmarkAndAppendToForest.py %s'%(arg))
    os.system('python /transfer/myphd_TowardsTransferInparallelFrames/RandomForestTests/testRandomForest.py %s %s %s'%(arg, iterationIndex,imagesRange))
    print 'executed \'testRandomForest.py\' '
    #import testRandomForest


      #print 'waiting ..forestPrediction.txt'
      #cv.WaitKey(0)
    ...
    ###import
    #Call python testRandomForest.py
    ###import


    print 'landmarkDisplacementsAccrossRange + mode --> decisionTreeOfThisRange : ',decisionTreeOfThisRange

    #readback forestPrediction and overRide variable 'testAveragedDisplacementsListForThisFrame'
    l=[]
    ll=[]

    while not(os.path.isfile(pathToDecisionTrees+'forestPrediction_%s_%d.txt'%(imagesRange,iterationIndex))) :
        print '..Waiting for all of the ranges\' forestPredictions to be exported by all instances of \'testInitASM.py'
        cv.WaitKey(1000)
    with open(pathToDecisionTrees+'forestPrediction_%s_%d.txt'%(imagesRange,iterationIndex)) as fp:
        lines = fp.readlines()
        lines = lines[0].split(',')
        firstEl = lines[0].split('[')[1]
        lastEl = lines[-1].split(']')[0]
        print lastEl
        lines[0] = firstEl
        lines[-1] = lastEl
    ...
    print 'Before'
    print testAveragedDisplacementsListForThisFrame #it's not 'forthisframe'
    #testAveragedDisplacementsListForThisFrame overriding
    testAveragedDisplacementsListForThisFrame = ll
    print 'After'
    print testAveragedDisplacementsListForThisFrame
    ...
    '''
     Now we need to save this 'decisionTreeOfThisRange' out, and run for the rest of the forest's decision trees
     and save these out too.
     When we're done, we 'll stack all the 'per landmark' decision trees so as to create a forest
     as in testdataset.csv.
     If we create a forest for each landmark, we can then calculate 'y_pred' the best possible/most accurate prediction 'per landmark',
     and save to testAveragedDisplacementsListForThisFrame and use this prediction to displace accordingly per frame / all landmarks.
     Do all of this and come back and continue
    '''

    '''
    f = open("demofile.txt", "a")

    #landmark_1_ChosenDisplacement_f1, landmark_1_ChosenDisplacement_f2, landmark_1_ChosenDisplacement_f2, landmark_1_ChosenDisplacement_f1, mode_at_this_range
    #first line of the testdataset

    for i in range(modeSelected.size()+1): #for every landmark out of the 43

        f.write(landmark_1_ChosenDisplacement_f1)
    ...
    #displace all shapes on all the frames for this iteration
    for frame in range(0,len(self.target_images),1):
      s = Shape([])

      for p_num, pt in enumerate(self.allInitializedShapes[frame].pts):#save each frame's shape displacements pointsOffsets as a list

        norm = self.allInitializedShapes[frame].get_normal_to_point(p_num)
        p = self.allInitializedShapes[frame].pts[p_num]
        side=0
        new_p_centered = Point(p.x + side*-norm[1], p.y + side*norm[0])
        new_p_centered_xx_offset = int(new_p_centered.x + (testAveragedDisplacementsListForThisFrame[0][p_num])*norm[0])
        new_p_centered_yy_offset = int(new_p_centered.y + (testAveragedDisplacementsListForThisFrame[0][p_num])*norm[1])

        s.add_point(Point(new_p_centered_xx_offset, new_p_centered_yy_offset))#, curBest



      #align new found shape point positions to the mean shape, and then use this to calculate the parameters of the current shape
      new_shape = s.align_to_shape(Shape.from_vector(self.asm.mean), self.asm.w)

      #start from mean shape
      new = self.asm.mean

      #dot all 5 model evecs  with (new shape-mean shape)..
      #..to derive b shape and pose parameters.
      #Update final new shape by incrementaly adding the to the current 'new' (starting from the mean shape ..:
      #the constrained b to 3 standard deviation and applied b parameters to each
      #of the evecs

      for i in range(len(self.asm.evecs.T)):
        #update parameters to fit the current shape

        #print len(new_shape.get_vector())
        #cv.WaitKey()
        #print len(self.asm.mean)
        #cv.WaitKey()

        #calculate/update the parameters of the current shape
        ...
        #use parameters to calculate new shape model from the mean shape adjusted to the b parameters calculated
        ...
        #align the new found shape constrained calculated positions to the raw new positions returnt found and returnt from profile model calculation
        #update each frame's mean shape
        self.allInitializedShapes[frame] = Shape.from_vector(new).align_to_shape(s, self.asm.w)
