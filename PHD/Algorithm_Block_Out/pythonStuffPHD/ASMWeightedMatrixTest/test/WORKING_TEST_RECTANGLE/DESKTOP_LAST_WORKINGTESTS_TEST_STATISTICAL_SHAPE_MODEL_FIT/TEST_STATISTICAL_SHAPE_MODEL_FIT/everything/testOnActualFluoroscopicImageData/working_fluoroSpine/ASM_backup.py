...
class FitModel:
  ...
  """
  def __init__(self, asm, image, originalShapes, t=Point(10,-5)):#x,y #5,85 for c3
    ...
    self.currentSearchPyramidLevel=0
...
####cv.WaitKey()
  ...
  def findtrainingGmean(self, testImageNameCounter, AllImages_NormalizedtmpLandmarkDerivativeIntensityVector, gaussianMatrix):
      ...
      for imagelevel in range(pyramidLevels):
          ...
          for asmshape in self.originalShapes:#self.asm.shapes !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 3 for now
              ...
              cv.Copy(self.g_image[ (testImageNameCounter-1) +(imagelevel*pyramidLevels )], trainingSetImage)
  ...
  def findcurrentShapeGmean(self, currentshape, One_NormalizedtmpLandmarkDerivativeIntensityVector, gaussianMatrix, currentSearchPyramidLevel):
      ...
      self.findLandmarkNormIIntesityVec (gMeanFlag, currentshapePointVector, currentshape, self.image, curShape_NormalizedtmpLandmarkDerivativeIntensityVector,gaussianMatrix, self.g_image[currentSearchPyramidLevel])
...
#                  ####cv.WaitKey()
      ...
      for i in range(9):
          ...
          cv.NamedWindow("testGimage", cv.CV_WINDOW_AUTOSIZE)
      ...
      ####print"\n np.array(y_j_mean)=%s"%(np.array(y_j_mean[0]).flatten())
          ...
          imgtmp = cv.CreateImage(cv.GetSize(self.image), self.g_image[currentSearchPyramidLevel].depth, 1)
          cv.Copy(self.g_image[currentSearchPyramidLevel], imgtmp)
          ...
          for t in drange(-6, 7, 1):#tangent not really (as at 1st currently, it chooses the directions side along normal and then from there onwards it searches around for nearby strong intensity edges to fit to)
              ...
              cv.NamedWindow("newPoint_NormalLine_CheckedAgainstTargetImage", cv.CV_WINDOW_AUTOSIZE)
              ...
              targetImageTocheckAgainst=self.g_image[currentSearchPyramidLevel] #self.image
...
return correctionVectorForAllShapes
  ...
  def do_iteration(self, scale, iterationIndex, GTrainingMeansCalculated):
    ...
    self.calcMaxPtFromMahalanobisModelProfile(self.shape, scale, iterationIndex, GTrainingMeansCalculated)
