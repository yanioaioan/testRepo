...
class FitModel:
  ...
  def __init__(self, asm, image, targetImageName, originalShapes, t=Point(0,0)):#0,0 was working
    ...
    self.savedBestIndicesPreviousIteration=[]##the list containing the points from 1 iteration back (previous iteration)
    self.totallandmarksAdjustedCounterPrev=0

    self.savedBestIndicesPreviousPreviousIteration=[]#the list containing the points from 2 iterations back
...
#######cv.WaitKey()
  ...
  def calculateIthImageLandmarkDerivIntensityVec(self, p, norm, greyImage, gaussianMatrix, _searchProfiles, IthImage_NormalizedtmpLandmarkDerivativeIntensityVector, gaussianWindow):
	  ...
	  #for t in drange(-3, 4, 1):#tangent MATRIX CHANGED
	  ...
	  #. The horizontalElementVectorSum  here is the Frobenius norm i.e. the euclidean norm if you treat the matrix as a single long vector
  ...
  def findtrainingGmean(self, testImageNameCounter, AllImages_NormalizedtmpLandmarkDerivativeIntensityVector, gaussianMatrix):
	  ...
	  for asmshape in self.originalShapes:#self.asm.shapes !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 3 for now
	      ...
	      #dirname="images" # NOT USED ANYMORE as we read from sys.argv[2]
...
#                  #######cv.WaitKey()
      ...
      CHOSEN_BEST = cv2.imread("current_targetImageTocheckAgainst.jpg", cv2.IMREAD_GRAYSCALE )#cv2.IMREAD_COLOR
	  ...
	  if iterationIndex==0:
	      ...
	      self.savedBestIndicesPreviousPreviousIteration.append(-1)
	  ...
	  whisker_center_plus_point = Point(p.x + (side_center_plus+(offsetByPixels/2.0))*norm[0], p.y + (side_center_plus+(offsetByPixels/2.0))*norm[1])
	  ...
	  for side in range(-6, 6):
	    ...
	    for t in drange(-currentSearchPyramidLevel if -currentSearchPyramidLevel > min_t else min_t, \
			     currentSearchPyramidLevel if currentSearchPyramidLevel < max_t else max_t , 1):
		...
		continue
	      ...
	      cv2.circle(CHOSEN_BEST, ( x, y ), 1, (255,255,255))

	      cv2.namedWindow("CHOSEN_BEST", cv.CV_WINDOW_NORMAL)
	      cv2.imshow('CHOSEN_BEST',CHOSEN_BEST)
	      #cv.WaitKey()
	      #print x, y, targetImageTocheckAgainst.width, targetImageTocheckAgainst.height
	      if targetImageTocheckAgainst[y-1, x-1] > max_edge:
		max_edge = targetImageTocheckAgainst[y-1, x-1]
...
dientIntensity)
      ...
      #If differ by 10% or less of the total element number of the list, which mean
      ...
      totallandmarksAdjustedCounter=0#counts the number of landmarks changed position from i to i+1 iteration
      totallandmarksAdjustedCounter2=0#counts the number of landmarks changed position from i+1 to i+2 iteration

      print "length of self.savedBestIndicesCurrentIteration=%s"%(len(self.savedBestIndicesCurrentIteration))
      print "--- self.savedBestIndicesCurrentIteration=%s"%((self.savedBestIndicesCurrentIteration))
      print "length of self.savedBestIndicesPreviousIteration=%s"%(len(self.savedBestIndicesPreviousIteration))
      print "--- self.savedBestIndicesPreviousIteration=%s"%((self.savedBestIndicesPreviousIteration))
      print "length of self.savedBestIndicesPreviousPreviousIteration=%s"%(len(self.savedBestIndicesPreviousPreviousIteration))
      print "--- self.savedBestIndicesPreviousPreviousIteration=%s"%((self.savedBestIndicesPreviousPreviousIteration))
      print "zip(self.savedBestIndicesCurrentIteration, self.savedBestIndicesPreviousIteration, self.savedBestIndicesPreviousPreviousIteration=%s"%(zip(self.savedBestIndicesCurrentIteration, self.savedBestIndicesPreviousIteration, self.savedBestIndicesPreviousPreviousIteration))
      #cv.WaitKey()

      for i, j, k in zip(self.savedBestIndicesCurrentIteration, self.savedBestIndicesPreviousIteration, self.savedBestIndicesPreviousPreviousIteration):
	  ...
	  if j!=k:
	      totallandmarksAdjustedCounter2+=1
	      '''
	      print "j=%s"%(j)
	      #cv.WaitKey()
	      print "k=%s"%(k)
	      #cv.WaitKey()
	      print "j different then k"
	      #cv.WaitKey()
	      #print "totallandmarksAdjustedCounter2=%d"%(totallandmarksAdjustedCounter2)
	      '''


      '''
      print "totallandmarksAdjustedCounter=%s"%(totallandmarksAdjustedCounter)
      print "totallandmarksAdjustedCounter2=%s"%(totallandmarksAdjustedCounter2)
      #cv.WaitKey()
      print "self.totallandmarksAdjustedCounterPrev=%s"%(self.totallandmarksAdjustedCounterPrev)
      ...
      #>5 as we want it to force it to run for a few iterations (at least 5),
      #as it has been observed that iteration 1 is not updating (possibly cause mahalanobis spatial sitance is same as iteration 0)
      #if iterationIndex>5 and (totallandmarksAdjustedCounter <= float(1)/10 * self.totallandmarksAdjustedCounterPrev):

      if iterationIndex>5 and (totallandmarksAdjustedCounter2 <= float(1)/10 * totallandmarksAdjustedCounter):
      ...
      #save deep copy savedBestIndicesPreviousIteration & savedBestIndicesPreviousPreviousIteration

      #NOTE:CORRECT ORDER OF COPYING
      #First deepcopy savedBestIndicesPreviousIteration to savedBestIndicesPreviousPreviousIteration &
      # then deepcopy savedBestIndicesCurrentIteration to savedBestIndicesPreviousIteration
      if iterationIndex>=2:
	  #cv.WaitKey()
	  #print "iterationIndex=%s"%(iterationIndex%2)
	  self.savedBestIndicesPreviousPreviousIteration = copy.deepcopy(self.savedBestIndicesPreviousIteration)
