...
def main():
  ...
  for i in range (len(shapes)):
      ...
      finalpath=''
      for pathSubPart in range(len(pathSplit)-2):
      ...
      #move to the next available image, say if frame 10 is missing, but frame 9 and frame 11 are there      

      while len(currentImageName) == 0:
      ...
      print os.path.join(dirname, "grey_image_%d.png"%(testImageNameCounter))
      ...
      #except:
      #  print os.path.join(dirname, "grey_image_%d.jpg doesn't exist so don't load it"%(testImageNameCounter))
  ...
  for shape in shapes:
      ...
      #targetimg = cv.LoadImage(imageNames[imageCounterColor-1])
      for i,p in enumerate(shape.pts):
          tmpP = Point(p.x, p.y)

          #print imageNames[imageCounterColor-1]
  ...
  for iterationIndex in range(15):
    scale=2
  ...
  whereToExport = (str(sys.argv[1])+"/exportedPointsForEachStaticFrame/"+"frame"+ str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]) + str('/') +"_frames__scale_2_frame_"+str(image_landmarkNumber)+".pts")
  ...
  #print splitExt[0]
  image_landmarkNumber = splitExt[0]
  whereToExport = (str(sys.argv[1])+"/exportedPointsForEachStaticFrame/"+"frame"+ str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]) + str('/') +"_frames__scale_1_frame_"+str(image_landmarkNumber)+".pts")
  exportSegmentationPoints(m, images, whereToExport)

  #Reset Gmean&Covariances once before each pyramid Level
  resetTrainingGmeaAndCovariances=1;
  for iterationIndex in range(5):
  ...
  #print splitExt[0]
  image_landmarkNumber = splitExt[0]
  whereToExport = (str(sys.argv[1])+"/exportedPointsForEachStaticFrame/"+"frame"+ str(sys.argv[2].split('/')[-1:][0].split('.')[0].split('_')[-1]) + str('/') +"_frames__scale_0_frame_"+str(image_landmarkNumber)+".pts")
  exportSegmentationPoints(m, images, whereToExport)
