...
####cv.WaitKey()
  ...
  def getCorrectedLandmarkPoint(self):
      ...
      for shape in self.asm.shapes:#self.originalShapes
          ...
          #s=[]
          #shape.add_point(Point(100,100))
          #shape.add_point(Point(200,200))
          #shape.add_point(Point(300,300))

          #print "asm shape x&y length flattend (..so divide by 2 to get the total coordinate points) : %s"%( len(shape.get_vector()) )
          #print "asm shape : %s"%( shape.get_vector() )
          #cv.WaitKey()

          '''get a vector of all landmark points of this shape'''
          oneTrainingImageVector=Shape.from_vector(shape.get_vector())#[320,185,309,196,292,203,276,209,264,217,256,237,261,252,275,255,294,243,308,238,327,240,345,241,342,224,336,217,335,209,348,203,359,210,369,214,376,206,392,211,400,215,404,214,402,198,399,189,390,181,389,172,383,160,373,147,354,145,338,161]
